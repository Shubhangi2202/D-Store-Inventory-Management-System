	...
	if (!empty($registerFullName) && !empty($registerUsername) && !empty($registerPassword1) && !empty($registerPassword2)) {
		...
		if ($usernameCheckingStatement->rowCount() > 0) {
			...
			if ($registerPassword1 !== $registerPassword2) {
				...
				$insertImageStatement = $conn->prepare($insertImage);
				$insertImageStatement->execute(['image' => $image]);
				...
				echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Image Inserted Successfully.</div>';
				exit();
			}
		}
	} else {
